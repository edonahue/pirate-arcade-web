...
test.describe('Game Load Performance', () => {
  test('should load Cannonball Clash with proper metrics', async ({ page }) => {
    ...
    const totalPageToPythonReady = metrics['python-ready'] - metrics['page-script-start'];
    const totalBootToGameReady = metrics['game-ready'] - metrics['boot-start'];
    const pygameInstallDuration = metrics['pygame-install-end'] - metrics['pygame-install-start'];
    const archiveFetchDuration = metrics['archive-fetch-end'] - metrics['archive-fetch-start'];
    const archiveExtractDuration = metrics['archive-extract-end'] - metrics['archive-extract-start'];
    const displayInitDuration = metrics['display-init-end'] - metrics['display-init-start'];
    const readyToLoaderHidden = metrics['loader-hidden'] - metrics['game-ready'];
    ...
    await page.evaluate(() => {
      window.lastTestMetrics = {
        ...
        metrics: window.PirateArcadeMetrics ? window.PirateArcadeMetrics.getAll() : {}
      };
    });
    ...
    if (totalPageToPythonReady > 30000) {
      console.warn(`⚠️  Slow total page to Python ready: ${totalPageToPythonReady}ms`);
    }
    ...
    const hasRuntimeError = await page.evaluate(() => {
      return !!window.PirateArcadeLoading && 
             document.getElementById('game-loading')?.classList.contains('game-error');
    });
  ...
  test("should load Treasure Cove with proper metrics", async ({ page }) => {
    const game = new BrowserGame(page, "treasure-cove");
    await game.goto();
    await game.waitForReady();

    // Check that all expected metrics are present
    const metrics = await game.getMetrics();
    expect(metrics).toHaveProperty("page-script-start");
    expect(metrics).toHaveProperty("pythons-js-requested");
    expect(metrics).toHaveProperty("python-ready");
    expect(metrics).toHaveProperty("boot-start");
