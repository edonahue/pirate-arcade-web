...
- **`/play/` prewarm**: Browser-playable game CTAs (both GameCard and standalone) have `data-game-id`, `data-browser-playable="true"`, `data-game-page` (same-origin game URL), and `data-game-archive` (versioned `.tar.gz` URL). Hovering/focusing/touching triggers a single `<link rel="prefetch">` insertion per URL and a `WARM_CACHE` postMessage to the SW controller. Prewarm fires only for browser-playable games; there are no desktop-only games on the site.
