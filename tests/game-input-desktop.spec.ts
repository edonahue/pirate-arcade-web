...
import {
  ...
  performConfiguredPrimaryAction,
...
interface GameSpec {
  ...
  name: string;
  path: string;
  /** Primary action key (Enter for pong, Space for breakout/asteroids). */
  actionKey: string;
...
const GAMES: GameSpec[] = [
  {
    ...
    actionKey: "Enter",
    testSequence: ["ArrowUp", "ArrowDown"],
  },
  {
    id: "treasure-cove",
    name: "Treasure Cove",
...
for (const game of GAMES) {
  test.describe(`${game.name} - desktop input`, () => {
    ...
    test("mouse click properly focuses canvas and enables keyboard", async ({
      ...
      // Send the configured primary action — should not throw or be lost
      await performConfiguredPrimaryAction(page, game.actionKey);
    ...
    test("no JavaScript dialogs during gameplay", async ({
      ...
      // Start game + drive gameplay to surface any alert/confirm/prompt
      await performConfiguredPrimaryAction(page, game.actionKey);
      await sendKeysAndRequireResponse(page, game.testSequence, 1500);
    ...
    test("Escape pauses game without errors", async ({ page }, testInfo) => {
      ...
      // Start the game with configured primary action
      await performConfiguredPrimaryAction(page, game.actionKey);
      await page.waitForTimeout(500);
